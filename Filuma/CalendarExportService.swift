...
enum CalendarExportService {
    ...
    static func syncNow(context: ModelContext, settings: UserSettings) throws {
        let calendar = try filumaCalendar(settings: settings)
        ...
        do {
            for block in exportable {
                ...
                if let eventId, let event = matchingEvent {
                    ...
                    event.startDate = block.startTime
                    event.endDate = block.endTime
                    event.notes = "Scheduled by Filuma"
                    try store.save(event, span: .thisEvent, commit: false)
    ...
    /// Remove the Filuma calendar entirely (export switched off).
    static func removeExportedEvents(context: ModelContext) {
        ...
        settings.filumaCalendarIdentifier = nil
    ...
    private static func filumaCalendar(settings: UserSettings) throws -> EKCalendar {
        if let identifier = settings.filumaCalendarIdentifier,
        ...
        settings.filumaCalendarIdentifier = calendar.calendarIdentifier
        return calendar
    }
}
