...
2. Credentials → **OAuth client ID**, type **iOS**, bundle ID
   `com.christoforakis.Filuma`.
3. Note the client ID (`xxxx.apps.googleusercontent.com`) and its reversed
   form — the reversed ID becomes a URL scheme in Filuma's Info.plist.
...
- **Auth (no SDK, no backend):** `ASWebAuthenticationSession` + PKCE against
  ...
  in the **Keychain** (`kSecClassGenericPassword`, service `com.christoforakis.Filuma.google`) —
  this Keychain wrapper becomes the shared pattern for any future OAuth
  integration.
- **Import (Google → Filuma):** `events.list` on the primary calendar, polled on
...
- **Export (Filuma → Google):** opt-in toggle, off by default; nothing is pushed
...
- **Loop guard:** exported Filuma events carry an extended property
  (`private.filuma=1`) and are excluded from import.
